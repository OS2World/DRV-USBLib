...
/* Most helpful debugging aid */
  #define assert(expr) ((void) ((expr) ? 0 : (printf("assert failed at line %d",__LINE__))))

  #define USES_PROC_FS	(defined(CONFIG_PROC_FS) && defined(CONFIG_VIDEO_PROC_FS))

/* We use new-style probe introduced in early 2.4 kernels */
  #define	USES_NEW_STYLE_PROBE	(((LINUX_VERSION_CODE >> 16) >= 2) && (((LINUX_VERSION_CODE >> 8) & 0xFF) >= 4))
...
typedef  struct
{
  ...
  char depth;
  char palette;
} video_picture;  /* Picture settings */

#define VID_TYPE_CAPTURE 2 
typedef struct
{
  int type;
  char name[30];
  ...
  int audios;
  int channels;
} video_capability;   /* Video capabilities */
#define VIDEO_TYPE_CAMERA 1
typedef struct video_channel 
{
  ...
  int tuners;
  int channel;
} vchan; /* May be used for tuner support */

typedef struct
{
  video_device vdev;  /* Must be the first field! */
  USBHANDLE dev;//struct usb_device *dev;
  ...
  DatastreamMarker_t emptyMarker; /* Optional empty marker for empty frames
             in an iso-stream. The empty marker is 
             inserted in ringbuffer when a 0 length
             frame is recieved in the iso stream and 
             the empty_marker_size > 0. Needed for 
             webcamgo driver */
  ...
  HSEM lock;//struct semaphore lock;
  ...
  video_picture vpic, vpic_old;  /* Picture settings */
  video_capability vcap;   /* Video capabilities */
  video_channel vchan; /* May be used for tuner support */
...
struct s_usbvideo_t
{
  ...
  HEV lock;//struct semaphore lock;    /* Mutex protecting camera structures */
  usbvideo_cb_t cb;   /* Table of callbacks (virtual methods) */
  video_device vdt;  /* Video device template */
                                 ...
                                 unsigned char vb)
{
  ...
  pf = (UCHAR*)(fr)->data + V4L_BYTES_PER_PIXEL*((iy)*VIDEOSIZE_X((fr)->request) + (ix));
  if (limiter)
  {
    *pf++ = 0;
    *pf++ = 0;
